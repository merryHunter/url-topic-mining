...
import smile.math.SparseArray;
    ...
    private Hashtable<String, Integer> tokenToIndex;
    ...
    public SparseDataset fitTransform(List<String> documentsNoTokens) {
        int nDocuments = documentsNoTokens.size();
        List<List<String>> documents = new LinkedList<>();
        for(int i = 0; i < nDocuments; i++){
            List<String>  doc = tokenize(documentsNoTokens.get(0));
            documents.add(doc);
        }
    ...
    public CountVectorizer fit(List<List<String>> documents) {
        ...
        tokenToIndex = new Hashtable<>(vocabulary.size());
    ...
    public Hashtable<String, Integer> getTokenToIndex() {
        return tokenToIndex;
    }
